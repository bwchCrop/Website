...
/* -----------------------------------------------------------------------------
 | 
 | File 		: Marge.php (v.1.1) Codeigniter 
 | Author 		: Deny (mailto: deny@kamargelap.id)
 | Date 		: 2017/08/23
 | Coor 		: Designcub3 
...
class Marge{
	...
	public function create_controllers($fileName,$action = NULL){
		...
		$text 	.= "\t\t}else{".PHP_EOL;
		$text 	.= "\t\t\t$"."data['result']   	= $"."this->mchildsubmisc->getAll()->result_array();".PHP_EOL;
		$text 	.= "\t\t}".PHP_EOL;
		$text   .= PHP_EOL;
		...
		if($data != NULL){
			...
			foreach($data['02'] as $row){
				...
				if($element == '4'){ // inputDate
					$element = "<input type=\"date\" name=\"".$field."\" id=\"".$field."\" class=\"form-control\"/>";
					...
					$element = "<input type=\"checkbox\" name=\"".$field."\" id=\"".$field."\" class=\"form-control minimal\"/>";
				...
				if($element != '9' && $element != '10' && $element != '11'){
                $text 	.= "\t\t\t\t\t<div class=\"form-group\">".PHP_EOL;
                $text 	.= "\t\t\t\t\t\t<label for=\"name\" class=\"form-control-label\" >".$row['menutemp_field_1']."</label>".PHP_EOL;
                $text 	.= "\t\t\t\t\t\t".$element.PHP_EOL;
                $text 	.= "\t\t\t\t\t</div>".PHP_EOL;
	...
	public function create_viewsedit($fileName,$data = NULL,$action = NULL){
		...
		if($data != NULL){
			...
			foreach($data['02'] as $row){
				...
				if($element == '4'){ // inputDate
					$element = "<input type=\"date\" name=\"v".$field."\" id=\"v".$field."\" class=\"form-control\" value=\"<?php echo date('Y-m-d',strtotime($"."result['".$tablename."_".$field."']));?>\"/>";
				}elseif($element == '5'){ // dropdown
					$element = "<input type=\"text\" name=\"v".$field."\" id=\"v".$field."\" class=\"form-control\" value=\"<?php echo $"."result['".$tablename."_".$field."'];?>\"/>";
				}elseif($element == '6'){ // imageupload
					$element = "<input type=\"text\" name=\"v".$field."\" id=\"v".$field."\" class=\"form-control\" value=\"<?php echo $"."result['".$tablename."_".$field."'];?>\"/>";
					...
					$element = "<input type=\"checkbox\" name=\"v".$field."\" id=\"v".$field."\" class=\"form-control minimal\" value=\"<?php echo $"."result['".$tablename."_".$field."'];?>\"/>";
				}elseif($element == '9' || $element == '10' || $element == '11'){
					$element = "";
				}else{ // inputText
					$element = "<input type=\"text\" name=\"v".$field."\" id=\"v".$field."\" class=\"form-control\" value=\"<?php echo $"."result['".$tablename."_".$field."'];?>\"/>";
	...
	public function create_table_db(){
		...
		$count  = 0;
		for($i=0; $i<$count; $i++){
			$fieldName 	= $this->input->post('value-'.$i,true);
			$fieldType 	= $this->input->post('type-'.$i,true);
			$fieldElmnt	= $this->input->post('element-'.$i,true);
			$fieldLength= $this->input->post('length-'.$i,true);
			$fieldMndtry= $this->input->post('mandatory-'.$i,true);

			$field .= ', '.strtolower($fieldName).' '.$fieldType.'('.$fieldLength.') '.$fieldMndtry ;
		}

		$query 	= 'CREATE TABLE '._PREFIX.$fileName.' ( id'.strtolower($fileName).' INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY '.$field.' )';
		$this->db->query($query);
	}
	...
	public function delete_content_file($data){
	    ...
	    // $file = "application/views/template/modal.php";
	    $file  = $data['path'];
	...
	public function encrypt($string = ''){
		$strencrypt = $this->CI->encrypt->encode($string, _ENCRYPTKEY);
	...
	public function decrypt($string = ''){
		...
		$strdecrypt = $this->CI->encrypt->decode($strdecrypt, _ENCRYPTKEY);
	...
	function date_Format($format = NULL, $month = NULL, $getDate = NULL){
		if($format == NULL)		{ $format 	= $this->thisFormat;}
