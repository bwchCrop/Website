      ...
      <form name="mAdd" method="POST" class="mAdd">
        ...
        <div class="modal-body">
            <div class="form-group">
              <label for="subcategory2" class="form-control-label">Item Category Name</label>
              <input type="text" class="form-control" id="subcategory2" name="subcategory2" >
            </div>          
            <div class="form-group">
              <label for="subcategory1" class="form-control-label">Group Item</label>
              <select id="subcategory1" name="subcategory1" class="form-control">
                  <?php foreach($res_group as $row){ ?>
                  <option value="<?php echo $row['id'] ?>"><?php echo $row['subcategory1'] ?></option>
                  <?php } ?>
              </select>
            </div>          
        </div>
...
$(function() {
  ...
  $(".mAdd").validate({
    ...
    messages: {
      subcategory2: {
        required: "Please provide a Item Category"
      },
      subcategory1: {
        required: "Please provide a Group Item"
      }
    },
    ...
    submitHandler: function(form) {
      $('.overlay').show();
      var subcategory2  = $("#subcategory2").val();    
      var subcategory1    = $("#subcategory1").val();    
      var modal        = $("#successModal");

      $.post("<?php echo site_url('add-itemcategory');?>",
        {subcategory2:subcategory2, subcategory1:subcategory1},
        function(data){
          $('.overlay').hide();
          $("#mAdd").modal('hide');
  ...
  $(".mView").validate({
    ...
    submitHandler: function(form) {
      $('.overlay').show();
      var subcategory2  = $("#vsubcategory2").val();
      var subcategory1     = $("#vsubcategory1").val();
      var ID            = $("#id").val();    
      var modal         = $("#successModal");

      $.post("<?php echo site_url('edit-itemcategory');?>",
        {subcategory2:subcategory2,id:ID,subcategory1:subcategory1},
        function(data){
          $('.overlay').hide();
          $("#mView").modal('hide');
          $('#successModal').modal({backdrop: 'static', keyboard: false})
  ...
  $(document).on('click','.closeMessage',function(e){
      ...
      window.location.replace("<?php echo base_url('itemcategory');?>");  
  });
